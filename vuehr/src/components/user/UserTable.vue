    ...
    <div class="table">
        ...
        <div class="handle-box">
            ...
            <el-button type="primary" class="handle-insert mr10">
              ...
              <router-link to="/createuser">新增</router-link>
            </el-button>
        </div>
        <el-table :data="tableData" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
            <el-table-column prop="name" label="中文名" >
            </el-table-column>
            <el-table-column prop="username" label="英文名" >
            </el-table-column>
            <el-table-column prop="hname" label="所在医院名称" width="200">
            </el-table-column>
            <el-table-column prop="role.nameZh" label="角色">
            </el-table-column>
            <el-table-column prop="phone" label="手机" >
            </el-table-column>
            <el-table-column prop="telephone" label="固定电话">
            </el-table-column>
            <el-table-column prop="address" label="地址">
            </el-table-column>
            <el-table-column prop="enabled" label="是否被禁用" width="100">
            </el-table-column>
       ...
       watch:{    //监听 select_word发生改变后，调用模糊查询方法
        ...
        methods: {
          ...
          handleCurrentChange(val){
            ...
            getData(){
                this.$axios.get(`http://localhost:5050/findAllSystemManagerUserPage?page=${this.cur_page}&size=${this.pagesize}`).then(data=>{
                    if(data){
                        console.log(3333333333333333333333333333333333333333333333333333333);
                        console.log(JSON.stringify(data.data.pageuserdata));
                        console.log(data.data.total+"total");
                        this.total=data.data.total;
                        this.tableData=data.data.pageuserdata;
                    }
                })
            ...
            likeselect(){
                //调用后台模糊查询接口
                this.$axios.get(`http://localhost:5050/getUserLike/${this.select_word}`).then(data=> {
                    if (data) {
           ...
           /*    console.log(row.did+"row.hidtest------------");*/
            ...
            handleDelete(index, row) {
                ...
                this.$axios.delete(`http://localhost:5050/deleteUser/${row.id}`).then(response=>{
