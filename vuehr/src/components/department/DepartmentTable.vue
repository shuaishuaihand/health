    ...
    <div class="departmenttable">
        ...
        <!-- <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>  {{$route.params.hid}}{{this.hospital.hname}}科室信息列表1111&lt;!&ndash;{{this.tableData[0].hid}}&ndash;&gt;</el-breadcrumb-item>
            </el-breadcrumb>
        </div>-->
        ...
        methods: {
          ...
          getData(){
              this.hid=this.$store.state.user.hid;
                console.log(this.$store.state.user.hid+"123574697947");
                ...
                this.$axios.get(`http://localhost:5050/findAllDepartmentPageByHid/${this.hid}?page=${this.cur_page}&size=${this.pagesize}`).then(data=>{
                    if(data){
            ...
            likeselect(){
                //调用后台模糊查询接口
                this.$axios.get(`http://localhost:5050/getDepartmentLike?selectword=${this.select_word}&hid=${this.hid}`).then(data=> {
                    if (data) {
          ...
          handleDepartmentCreate() {
            ...
            handleDelete(index, row) {
                console.log(row.did+"row");
                this.$axios.delete(`http://localhost:5050/deleteDepartment/${row.did}`).then(response=>{
